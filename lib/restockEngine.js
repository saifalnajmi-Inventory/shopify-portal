...
export async function checkRestockRule(variant, prevQty, product, globalSettings = {}) {
  ...
  if (prevQty <= 0 && currentQty > 0) {
    ...
    // Auto-resolve any stale OOS/low-stock cards for this variant — it's genuinely
    // back in stock now, so they'd otherwise sit open and contradict live Shopify data.
    // Matched on productId + variantName (not sku — sku is often empty/inconsistent
    // and would silently fail to match), using the same expression used when the
    // original OOS/low_stock notification was created.
    try {
      await db.notification.updateMany({
        where: {
          productId:   variant.productId,
          variantName: variant.title !== 'Default Title' ? variant.title : null,
          type:        { in: ['oos', 'low_stock'] },
          status:      { not: 'resolved' },
        },
        data: { status: 'resolved', resolvedAt: new Date(), resolvedBy: 'system:auto-restock' },
      })
    } catch (e) {
      console.error('[restockEngine] auto-resolve stale OOS notifications failed:', e.message)
    }
