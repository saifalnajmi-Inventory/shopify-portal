...
/**
 * Fetch products created on/after a timestamp (ISO8601).
 * Lightweight incremental pull for "fetch new uploads" — same fields as
 * fetchAllProducts but filtered by created_at_min so we only pull recent ones.
 * No status filter → includes draft uploads (e.g. SPF Wizard pushes).
 */
export async function fetchProductsCreatedSince(createdAtMin) {
  const products = []
  const fields = [
    'id','title','handle','status','product_type','vendor','tags','body_html',
    'images','variants','options','published_at','created_at','updated_at',
  ...
  let url = `${baseUrl()}/products.json?limit=250&fields=${fields}`
  if (createdAtMin) url += `&created_at_min=${encodeURIComponent(createdAtMin)}`
...
/** Set inventory level for a specific variant at a location (absolute) */
...
/**
 * Adjust inventory RELATIVELY at a location.
 * Use this instead of setInventoryLevel when online orders may have already
 * decremented Shopify stock — this preserves those decrements.
 *
 * e.g. adjustment = -2 means "2 more units were sold in the physical store"
 */
export async function adjustInventoryLevel({ inventoryItemId, locationId, adjustment }) {
  const res = await shopifyFetch(`${baseUrl()}/inventory_levels/adjust.json`, {
    method: 'POST',
    body: JSON.stringify({
