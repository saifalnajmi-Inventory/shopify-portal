 ...
 *   1.5 barcode_to_sku — POS barcode === Shopify variant SKU
 *       (products pushed by SPF Wizard store the POS barcode as Shopify SKU)
...
export async function runPosMatch() {
  ...
  const matchTypeCounts = { barcode: 0, barcode_to_sku: 0, sku: 0, sku_in_title: 0, name_token: 0 }
  ...
  const rows = posProducts.map(pos => {
    ...
    // Priority 1.5: POS barcode === Shopify variant SKU
    // SPF Wizard stores the POS barcode as the Shopify SKU when creating new products
    if (!variant && pos.barcode?.trim()) {
      const v = bySku.get(pos.barcode.trim().toUpperCase())
      if (v) { variant = v; matchType = 'barcode_to_sku' }
    }
